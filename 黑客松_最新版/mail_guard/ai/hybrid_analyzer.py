...
def _merge(slm: AnalysisResult, gemini: AnalysisResult) -> AnalysisResult:
    ...
    average_score = round(slm.risk_score * 0.1 + gemini.risk_score * 0.9)

    if both_explicit:
        risk_score = max(85, average_score)
    elif max(slm.risk_score, gemini.risk_score) >= 60:
        risk_score = min(84, max(60, average_score))
    elif all(
        result.category is EmailCategory.SAFE and result.risk_score < 30
        for result in (slm, gemini)
    ):
        risk_score = min(29, average_score)
    else:
        risk_score = min(59, max(30, average_score))
